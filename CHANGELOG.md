...
- **复盘重构「历史对弈回看」**：个人棋谱复盘改为整页回看——逐步 / 首末手回放 + **逐手黑方胜率曲线**
  （KataGo 轻量评估，跨手补齐、引擎不可用可中断）；顶栏去掉返回箭头 / 标题 / 一次性 AI 建议，交互更顺。
- **试下（人工续弈）**：任意历史局面进入试下，黑白交替自由落子推演（悔棋 / 停手=PASS / 点目 / 返回）；
  连续两停手自动弹出点目结果，返回后精确恢复原历史局面；历史回看双返回或「退出」直达首页。

### Changed

- **等级档差**：级内每档固定 10 分（18级=0、17级=10…），段位区档差逐档 +30%
  （`10×1.3^(i−16)` 向下取整：10/13/16/21/28/37/48/62/81/106，9段累计 582）。
- **快速对弈（人机）即时计分**：同档胜分 `W=ceil(档差/3)`（约连赢 3 场晋级）、负分 `=W/2`
  （约连输 6 场降级），按对手相对档差 `d` 加权：胜 ×`clamp(1+0.2d, 0, 2)`、负 ×`clamp(1−0.2d, 0, 2)`；
  对手低自己 5 档获胜不得分、高自己 5 档失利不扣分；和棋 / 弃局 0 分。
- **大赛按名次一次性结算**：冠军 = 赛事最强对手「上一档」档差 X，亚军 X/2、四强 X/4、八强 0（向下取整）；
  提前退赛无任何积分 / 胜负 / 参赛统计。
- **升降级**：积分跌破当前档底线即降级（可被负局扣分触发），钳 0、最低 18级、最高 9段；积分由 0 可叠加增长。
- **对局收束**：终局成绩单去掉「再来一局」，结束后人机局直接回首页、生涯局返回赛程页完成结算与晋级提示；
  胜率曲线面板抽取为共享组件（对局页 / 回看页共用）。
