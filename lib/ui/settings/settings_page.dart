...
class SettingsPage extends ConsumerWidget {
        ...
        child: ListView(
          ...
          children: [
            ...
            _SettingsTile(
              ...
              onTap: () => Navigator.of(
                context,
              ).push(slideRightRoute<void>(const EditNamePage())),
            ),
            ...
            _SettingsTile(
              ...
              onTap: () => Navigator.of(
                context,
              ).push(slideRightRoute<void>(const BoardSizePage())),
            ),
            const SizedBox(height: 10),
            _SettingsTile(
              title: const Text('对弈规则'),
              subtitle: Text(
                '${settings.rule.label} · 贴目 ${_fmtKomi(settings.komi)}',
              ),
              chevron: true,
              onTap: () => Navigator.of(
                context,
              ).push(slideRightRoute<void>(const RulePage())),
            ),
            const SizedBox(height: 10),
            _SettingsTile(
              ...
              onTap: () => Navigator.of(
                context,
              ).push(slideRightRoute<void>(const MoveStylePage())),
            ),
            const SizedBox(height: 16),
            _SectionLabel('通用'),
            _SettingsTile(
              ...
              onTap: () => Navigator.of(
                context,
              ).push(slideRightRoute<void>(const SoundPage())),
            ),
            const SizedBox(height: 10),
            _SettingsTile(
  ...
  Future<void> _confirmRebirth(BuildContext context, WidgetRef ref) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('重生'),
        content: const Text('重生将初始化棋手信息与全部进度（名称、段位、积分、对局记录、赛事、打卡与做题进度），确定继续？'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('取消'),
