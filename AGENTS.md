...
| 对弈 | 两种模式：**生涯模式**（报名随机生成"对应个人段位"的大赛，按大赛积分升降级）与**人机模式**（自由选择对手段位/尺寸/规则对弈，胜负按对手档差加权即时计分、共同驱动个人积分与段位）。共用对局页：棋盘绘制、悔棋/认输/PASS、实时形势判断（ownership 热力图）、AI 建议下一步、终局数子/数目、保存棋谱 |
| 棋谱 | 棋谱库只展示**休闲观赛保存 + 本地导入 SGF** 两类棋谱（个人对局不再于此列出，回看入口在首页「历史记录」）；AppBar 右上角「导入 SGF」（图标按钮）从本地选文件入库；条目点击进入回看页：逐步/首末手回放、逐手黑方胜率曲线（轻量评估）、任意手位势力范围，可进入「试下」自由推演（悔棋/停手 PASS/点目/返回），双返回或「退出」直达首页 |
| 功课 | 基础规则新手指引（12 步交互引导：棋盘/气与提子/禁入点/连接分断/做眼/打劫/点目/停一手，见 `beginner_guide.dart`）/ 定式布局（常见定式 SGF 序列+讲解）/ 死活题（优先，答题判定+正解讲解+进度） |
...
统一封装于 `lib/ui/common/responsive.dart`，所有页面横屏/平板适配一律复用它，禁止各自硬编码方向判断：

- **断点**：`Breakpoints.compact=600` / `medium=840` / `expanded=1200`；`twoPane=720` 为启用两栏的阈值（覆盖手机横屏与平板）。`isWideLayout(context)` 判定宽屏/横屏。
- **棋盘页**（对局/观赛/复盘/死活题/定式/新手指引）用 `AdaptiveBoardLayout`：竖屏保持原有上下堆叠，宽屏/横屏自动变为**棋盘在左、状态卡/胜率/操作按钮在右**（默认 1:1，右栏可滚动）；底部含列表的页面用 `bottomExpanded: true`（定式匹配列表）。竖屏分支必须与历史布局等价，保证现有 widget 测试（`360×800`）零回归。
- **列表 / 设置 / 表单 / 配置页**用 `CenteredContent(maxWidth: …)` 居中限宽（快速匹配/休闲观赛 600、设置 640、选项 560、改名 500、棋谱/题库 720），避免平板上被拉伸过宽；配置类页面不做分栏。
- **首页横屏**：顶栏通栏，下方左右两栏（左 = 统计/每日一题/快速对弈/赛事/观赛等功能卡，右 = 快捷入口功能区 + 可滚动历史记录）。
- 新增含棋盘或长列表的页面时，按上述模式接入；不锁方向、不改竖屏行为。
...
8. **个人棋谱**：持久化所有个人对局（来源：生涯/人机），可复盘、可看势力范围、可 AI 分析下一手；入口在首页「历史记录」（棋谱页仅收观赛/导入）。
...
│   │   ├── beginner_guide.dart   # 基础规则新手指引：12 步引导数据 + 引擎（纯 Dart，可单测）
│   │   └── lesson_data.dart      # 定式布局目录（自编）
│   ├── storage/
│   │   ├── user_store.dart       # 用户档案（名称/段位/积分）持久化
│   │   ├── settings_store.dart   # 设置持久化
│   │   ├── record_store.dart     # 棋谱库（个人 + 观赛 + 导入，SGF 文件 + prefs 索引）
│   │   └── problem_store.dart    # 死活题进度（完成状态 + 尝试次数）持久化
│   └── ui/
│       ├── home/                 # 首页：用户条 + 四入口卡片
│       ├── play/                 # 对弈：模式选择/大赛列表/大赛详情/对局页
│       ├── record/               # 棋谱：观赛保存 + 导入 SGF 棋谱库 + 复盘页
│       ├── study/                # 功课：新手指引/定式/题库/答题页
│       ├── settings/             # 设置页
│       ├── board_widget.dart     # 棋盘绘制（CustomPainter）、手势落子、提子动画
│       ├── analysis_overlay.dart # 领地热力图 + 终局数字覆盖层 + AI 建议标注
│       ├── score_sheet.dart      # 数子/数目结果面板
│       └── common/               # 段位徽章、列表项、对话框、响应式布局等复用组件
│           └── responsive.dart   # 断点 + 居中限宽 + 棋盘页左右两栏自适应布局
...
- **档差**：级内（18级..1级，i=0..16）每档固定 10 分（18级=0、17级=10、16级=20…）；
  段位区域（1级->1段 起，i=17..25）档差在前一档差基础上 **+30%**，即 `10×1.3^(i−16)`
  向下取整（10/13/16/21/28/37/48/62/81/106；9段累计 582）。
...
2. **报名**：同一时刻**至多参加一场**；报名后进入赛程页，逐局对弈（对手按段位映射真实棋力，§8）。
   ...
   **自动模拟剩余赛程**并产生冠军。
3. **积分结算**：**赛事完结统一结算**——按名次一次性发放。封顶 `X` = 赛事最强对手
   「上一档」档差：**冠军 X / 亚军 X/2 / 四强 X/4 / 八强 0**（向下取整）。
   **提前退赛无任何积分/胜负/参赛统计**，历史记「退赛·0 分」。
4. **升降级**：`careerPoints` 达下一档阈值 → 晋升（保留绝对积分）；跌破当前档底线 → 降级
   （快速对弈负局扣分即可触发）。保护：积分不低于 0、段位不低于 18级、不高于 9段。
5. **人机模式**：自由选择对手段位/尺寸/规则/先后手对弈，即时结算个人积分：同档基础胜分
   `W = ceil(档差/3)`（约连赢 3 场晋级）、负分 `= W/2`（约连输 6 场降级），
   并按对手相对档差 `d = 对手档 − 自己档` 加权：胜 ×`clamp(1+0.2d, 0, 2)`、
...
`flutter_riverpod`、`shared_preferences`、`path_provider`、`file_picker`（棋谱页本地导入 SGF）。
